# dashboard_server.ps1 - Live Copy Trader Dashboard (local HTTP server)
# Runs at http://localhost:8765/  |  Stop: visit http://localhost:8765/stop or Ctrl+C

param([int]$Port = 8765, [int]$RefreshSec = 60, [int]$CTCacheMin = 5)

$ScriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$cfg       = Get-Content "$ScriptDir\config.json" | ConvertFrom-Json

$H    = @{ "APCA-API-KEY-ID" = $cfg.alpaca_key; "APCA-API-SECRET-KEY" = $cfg.alpaca_secret }
...
function HtmlEnc($s)    { [System.Web.HttpUtility]::HtmlEncode($s) }

# â”€â”€ Capitol Trades cache â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
$script:ctByPol    = @{}
$script:ctCacheAge = [datetime]::MinValue

function Refresh-CTCache {
    Write-Host "  [CT] Refreshing Capitol Trades cache..."
    ...
    if (Test-Path "$ScriptDir\trades_executed.json") {
        $raw = Get-Content "$ScriptDir\trades_executed.json" -Raw -ErrorAction SilentlyContinue
    ...
    foreach ($pol in $cfg.politicians) {
        ...
        try {
            $html = (Invoke-WebRequest -Uri "https://www.capitoltrades.com/trades?politician=$($pol.id)&pageSize=20&page=1" -UserAgent $UA -UseBasicParsing -TimeoutSec 10).Content
            ...
            foreach ($tm in ($m_ticker | Select-Object -First 15)) {
                ...
                $nType   = $m_txType | Where-Object { $_.Index -gt $tIdx } | Select-Object -First 1
                $nDate   = $m_txDate | Where-Object { $_.Index -gt $tIdx } | Select-Object -First 1
                $nVal    = $m_value  | Where-Object { $_.Index -gt $tIdx } | Select-Object -First 1
                if (-not $nType -or -not $nDate) { continue }
                if (($nType.Index - $tIdx) -gt 2000) { continue }
                $txId  = if ($nearId) { "$($pol.id)-$($nearId.Groups[1].Value)" } else { "" }
                $value = if ($nVal -and ($nVal.Index - $tIdx) -lt 2000) { try { [long]$nVal.Groups[1].Value } catch { 0 } } else { 0 }
                $trades.Add([pscustomobject]@{
                    txId   = $txId
                    ticker = $tm.Groups[1].Value.ToUpper()
                    action = $nType.Groups[1].Value.ToUpper()
                    date   = $nDate.Groups[1].Value
                    value  = $value
                    copied = $executed -contains $txId
                })
            }
            Write-Host "  [CT] $($pol.name): $($trades.Count) trades"
        } catch { Write-Host "  [CT] Error $($pol.name): $_" }
        $newCache[$pol.id] = $trades
        Start-Sleep -Milliseconds 700
    }
    $script:ctByPol    = $newCache
    $script:ctCacheAge = Get-Date
}
...
function Build-HTML {
    Write-Host "  [ALPACA] Fetching live data..."

    $acct      = Invoke-RestMethod -Uri "$BASE/account"                    -Headers $H -TimeoutSec 10
    $positions = Invoke-RestMethod -Uri "$BASE/positions"                  -Headers $H -TimeoutSec 10
    $orders    = Invoke-RestMethod -Uri "$BASE/orders?status=all&limit=50" -Headers $H -TimeoutSec 10
    $clock     = Invoke-RestMethod -Uri "$BASE/clock"                      -Headers $H -TimeoutSec 10
    ...
    $logLines = @()
    if (Test-Path "$ScriptDir\copy_trader.log") { $logLines = Get-Content "$ScriptDir\copy_trader.log" -Tail 80 }

    # Refresh CT cache if stale
    $cacheAge = (Get-Date) - $script:ctCacheAge
    if ($cacheAge.TotalMinutes -gt $CTCacheMin -or $script:ctByPol.Count -eq 0) {
        Refresh-CTCache
    }
    $ctByPol = $script:ctByPol
    $ctNextSec = [int][Math]::Max([double]0, ($CTCacheMin * 60) - $cacheAge.TotalSeconds)
    ...
    foreach ($p in ($positions | Sort-Object { [double]$_.unrealized_pl } -Descending)) {
        ...
        $uplC   = PLC $upl; $dayC = PLC $dayPct
        $posRows += "<tr><td><span class='sym'>$($p.symbol)</span></td><td class='num'>$($p.qty)</td><td class='num'>$([Math]::Round([double]$p.avg_entry_price,2).ToString('N2'))</td><td class='num'>$([Math]::Round([double]$p.current_price,2).ToString('N2'))</td><td class='num'>$([Math]::Round([double]$p.market_value,2).ToString('N2'))</td><td class='num' style='color:$uplC;font-weight:700'>$(Fmtc $upl)</td><td class='num' style='color:$uplC;font-weight:700'>$(Fmtp $uplPct)</td><td class='num' style='color:$dayC'>$(Fmtp $dayPct)</td></tr>"
    }
    $posRows += "<tr class='total-row'><td colspan='4'>TOTAL ($($positions.Count) positions)</td><td class='num'>$([Math]::Round($totalMV,2).ToString('N2'))</td><td class='num' style='color:$(PLC $totalUnrealPL);font-weight:700'>$(Fmtc $totalUnrealPL)</td><td class='num' style='color:$(PLC $totalUnrealPL);font-weight:700'>$(Fmtp $totalUnrealPct)</td><td></td></tr>"
...
<div class="pol-panel">
  ...
  <table style="width:100%;border-collapse:collapse;margin-top:12px">
    <thead><tr>
      <th style="$thStyle">Ticker</th><th style="$thStyle">Action</th>
      <th style="$thStyle">Date</th><th style="$thStyle">Value</th><th style="$thStyle">Status</th>
    </tr></thead>
    <tbody>$tradeRows</tbody>
  </table>
  <div style="margin-top:10px;text-align:right">
    <a href="https://www.capitoltrades.com/politicians/$($pol.id)" target="_blank" style="font-size:11px;color:#3b82f6">View on Capitol Trades &rarr;</a>
  </div>
</div>
"@
    }
    ...
    # Scheduler logs (screener / executor / monitor)
    ...
    $polBadges = ""
    foreach ($pol in $cfg.politicians) {
        $pc = PartyColor $pol.party; $pb = PartyBg $pol.party
        $polBadges += "<div style='background:$pb;border:1px solid $pc;border-radius:8px;padding:6px 14px;font-size:12px'><span style='color:#0f172a;font-weight:700'>$($pol.name)</span><span style='color:$pc;margin-left:6px;font-size:10px;font-weight:700'>$($pol.party)-$($pol.state)</span><span style='color:#64748b;margin-left:6px;font-size:10px'>$($pol.score)/100</span></div>"
    }
...
.refresh-btn{padding:6px 14px;border-radius:8px;background:#fff;border:1px solid #cbd5e1;color:#374151;font-size:12px;cursor:pointer;font-weight:600;text-decoration:none;display:inline-block}
.refresh-btn:hover{background:#f8fafc}
.stop-btn{padding:6px 12px;border-radius:8px;background:#fff;border:1px solid #e2e8f0;color:#94a3b8;font-size:11px;cursor:pointer;text-decoration:none;display:inline-block}
.stop-btn:hover{color:#dc2626;border-color:#dc2626}
.countdown{font-size:11px;color:#94a3b8;font-variant-numeric:tabular-nums}
.ct-cache{font-size:10px;color:#cbd5e1;margin-left:6px}
...
.progress-bar{height:3px;background:#e2e8f0;position:fixed;top:0;left:0;right:0;z-index:999}
.progress-fill{height:100%;background:linear-gradient(90deg,#3b82f6,#22c55e);transition:width 1s linear}
</style>
</head>
<body>
...
<div class="topbar">
  ...
  <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap">
    <span class="countdown">Auto-refresh in <span id="cd">$RefreshSec</span>s</span>
    <span class="ct-cache">CT data cached (next in <span id="ctcd">$ctNextSec</span>s)</span>
    <a href="/" class="refresh-btn">&#8635; Refresh Now</a>
    <a href="/stop" class="stop-btn" onclick="return confirm('Stop the dashboard server?')">&#9632; Stop Server</a>
...
<div class="pol-grid">$polPanels</div>
...
<div class="footer">
  ...
  <div class="fs" style="margin-left:auto">http://localhost:$Port/</div>
</div>
...
// Capitol Trades cache countdown
let ctS = $ctNextSec;
const ctcd = document.getElementById('ctcd');
const ctiv = setInterval(() => {
  ctS--;
  if (ctcd) ctcd.textContent = Math.max(0, ctS) + 's';
  if (ctS <= 0) clearInterval(ctiv);
}, 1000);
</script>
</body>
</html>
...
# Pre-warm Capitol Trades cache so first browser request responds instantly
Write-Host ""
Write-Host "  Warming Capitol Trades cache (fetching 5 politicians)..."
Refresh-CTCache
Write-Host "  Cache ready."
...
Write-Host ""
Write-Host "  CopyTrader Dashboard running at http://localhost:$Port/"
Write-Host "  Auto-refresh: every ${RefreshSec}s | CT cache: every ${CTCacheMin}min"
Write-Host "  Stop: visit http://localhost:$Port/stop  or press Ctrl+C"
Write-Host ""
...
try {
    while ($listener.IsListening) {
        ...
        if ($path -eq "/stop") {
            $msg   = "<html><body style='background:#f1f5f9;color:#1e293b;font-family:sans-serif;padding:40px;text-align:center'><h2>Dashboard server stopped.</h2><p style='color:#64748b;margin-top:12px'>You can close this tab.</p></body></html>"
            $bytes = [System.Text.Encoding]::UTF8.GetBytes($msg)
            $resp.ContentType = "text/html; charset=utf-8"
            $resp.ContentLength64 = $bytes.Length
            $resp.OutputStream.Write($bytes, 0, $bytes.Length)
            $resp.Close()
            Write-Host "  Stop requested. Shutting down."
            $listener.Stop()
            break
        }
        ...
        Write-Host "  Request: $($req.HttpMethod) $path"
        try {
            ...
            $errMsg = "<html><body style='background:#f1f5f9;color:#dc2626;font-family:monospace;padding:40px'><pre>Error: $_</pre></body></html>"
